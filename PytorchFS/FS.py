      ...
      def hook(module, input, output):
         ...
         for _targetNeuronIdx in _targetIndexes:
            ...
            for bit_pos in bit_positions:
               reverse_pos = len(bits) - 1 - bit_pos
               bits[reverse_pos] = str(int(not bool(int(bits[reverse_pos]))))
         ...
         _neurons = np.clip(_neurons, -1e6, 1e6) ## for mahalanobis
         _neurons = np.nan_to_num(_neurons, nan=0.0, posinf=1e6, neginf=-1e6) ## for mahalanobis
